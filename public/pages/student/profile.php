...
$currentProfileImageUrl = $currentProfileImage !== '' ? BASE_URL . '/public/assets/images/students/' . rawurlencode($currentProfileImage) : '';
    ...
    if (!app_validate_csrf_token($_POST['csrf_token'] ?? '')) {
        ...
        if ($username === '' || $firstName === '' || $lastName === '' || $email === '') {
            ...
            if (
                ...
                $uploadError = '';
                $uploadedFile = app_store_processed_image($_FILES['profile_image'], $_POST['profile_image_cropped'] ?? '', $uploadDir, 'user_' . $safeAdmission, $uploadError, 2 * 1024 * 1024);
...
<div class="student-page">
    ...
    <section class="student-layout-grid">
        <div class="student-panel">
            ...
            <form method="POST" action="" enctype="multipart/form-data" id="studentProfileForm">
                ...
                <div class="student-form-grid">
                    <div class="student-field full">
                        ...
                        <div class="student-file-picker">
                            <button type="button" class="student-file-btn" id="profile_file_btn">Choose New Photo</button>
                            <input type="text" class="student-input" id="profile_file_name" value="<?php echo $currentProfileImageUrl !== '' ? 'Current photo loaded' : 'No photo selected'; ?>" readonly>
                        </div>
                        <input type="file" name="profile_image" id="profile_image" class="d-none" accept=".jpg,.jpeg,.png,.webp">
                        <div class="square-cropper mt-3" id="student_profile_cropper" hidden>
                            <div class="square-cropper-stage" id="student_profile_cropper_stage">
                                <img id="student_profile_cropper_image" alt="Crop preview" draggable="false" style="display:none;">
                                <div class="square-cropper-frame"></div>
                            </div>
                            <div class="square-cropper-controls">
                                <label class="square-cropper-slider-row" for="student_profile_cropper_zoom">
                                    <span>Zoom</span>
                                    <input type="range" id="student_profile_cropper_zoom" class="square-cropper-slider" min="1" max="3" step="0.01" value="1">
                                    <span>3x</span>
                                </label>
                                <div class="square-cropper-actions">
                                    <button type="button" class="square-cropper-btn" id="student_profile_cropper_reset">Reset</button>
                                </div>
                            </div>
                            <div class="square-cropper-help">Drag the image within the fixed square to control how your profile photo appears.</div>
                        </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    window.initSquareImageCropper({
        formId: 'studentProfileForm',
        fileInputId: 'profile_image',
        hiddenInputId: 'profile_image_cropped',
        wrapperId: 'student_profile_cropper',
        stageId: 'student_profile_cropper_stage',
        imageId: 'student_profile_cropper_image',
        sliderId: 'student_profile_cropper_zoom',
        resetButtonId: 'student_profile_cropper_reset',
